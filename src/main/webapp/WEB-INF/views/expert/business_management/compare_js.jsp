...
<!-- 장기조제환자 현황 list, pagination handlebars -->
...
<!-- 주성분 검색 list, pagination handlebars -->
...
<!-- 약이름 검색 list, pagination handlebars -->
...
$(function() {
	...
	if($('title').text() == '제약사별 매입가 비교'){
		if(getCookieObjeck('mdName') != null){  // 매입가 비교 - 약이름 검색 부분 기본 설정
		...
		if(getCookieObjeck('basis') != null){  // 매입가 비교 - 주성분 검색 부분 기본 설정
		...
		addSelectList("<c:out value='${loginUser.chemistCode}'/>");  // 선택한 약 목록 기본 설정
	}
